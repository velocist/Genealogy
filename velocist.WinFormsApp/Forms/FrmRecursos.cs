...
namespace velocist.WinFormsApp.Forms {
    ...
    public partial class FrmRecursos : Form {
        ...
        private bool datagridUserActions = false;
        private bool searchRow = false;
        ...
        private void LoadTable(DataGridView dataGridView) {
            try {
                ...
                if (list != null)
                    dataGridView.LoadTable(list, true);
            } catch (Exception ex) {
                throw new Exception(ex.Message);
            }
        }
        ...
        private void BtnExport_Click(object sender, EventArgs e) {
            try {
                if (!this.CheckOpenForms<ModalExport<RecursoModel>>()) {
                    ModalExport<RecursoModel> frm = new((List<RecursoModel>)DgvData.DataSource);
                    frm.ShowDialog();
                }
            } catch (Exception ex) {
