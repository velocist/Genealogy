...
namespace Genealogy.AccesService {
	...
	public static class AccessServiceContainerExtensions {
		...
		public static void RegisterSqlServer<TContext>(this ContainerBuilder builder) where TContext : DbContext {
			_ = builder.RegisterType<velocist.DataAccess.SqlServer.UnitOfWork<TContext>>().As<velocist.DataAccess.SqlServer.Interfaces.IUnitOfWork<TContext>>().InstancePerLifetimeScope();
			_ = builder.RegisterType<velocist.DataAccess.SqlServer.GenericRepository<IEntity, TContext>>().As<velocist.DataAccess.SqlServer.Interfaces.IGenericRepository<IEntity, TContext>>().InstancePerLifetimeScope();
		}
