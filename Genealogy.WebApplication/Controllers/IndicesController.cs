...
namespace Genealogy.WebApplication.Controllers {
    ...
    public class IndicesController : Controller {
        ...
        private readonly IBaseService<IndiceModel, Indices, AppEntitiesContext> _service;
        #endregion
        ...
        /// </value>
        private ModalConfiguration<IndiceModel> ModalConfiguration { get; }
        ...
        public IndicesController(IStringLocalizer<SharedTranslations> sharedTranslations, IStringLocalizer<ViewsTranslations> viewTranslates, IDateTime date, IViewRender renderView, IBaseService<IndiceModel, Indices, AppEntitiesContext> service) {
            _logger = LogServiceContainer.GetLog<IndicesController>();
            ...
            ModalConfiguration = new ModalConfiguration<IndiceModel>("Indices") {
                Title = "Indices FS",
        ...
        public async Task<IActionResult> Create() {
            if (!User.Identity.IsAuthenticated)                 //string strRenderView = await _renderView.RenderAsync("AccessDenied.cshtml", model, new ViewDataDictionary(ViewData));
        ...
        [Authorize(Roles = "SuperAdmin")]
        [HttpPost]
        public async Task<IActionResult> Create(IndiceModel model) {
            string strRenderView;
            if (!User.Identity.IsAuthenticated)                 //strRenderView = await _renderView.RenderAsync("Error.cshtml", model, new ViewDataDictionary(ViewData));
                return await Task.FromResult(Unauthorized());
        ...
        [Authorize(Roles = "SuperAdmin")]
        [HttpPost]
        public async Task<IActionResult> Edit(IndiceModel model) {
            string strRenderView;
            InitView(Constants.Action.EDIT);
            try {
                if (ModelState.IsValid)
        ...
        //    try {
        //        if (id > 0) {
        //            IndicesModel model = new IndicesModel(_unitOfWork, id).GetAll();
        //            if (model != null) {
        //                ViewBag.Id = model.Id;
        ...
        //[Authorize(Roles = "SuperAdmin")]
        //[HttpPost]
        //public async Task<IActionResult> RemoveAll(IndicesModel model) {
        //    try {
        //        if (model != null) {
        //            if (!new IndicesModel(_unitOfWork, model.Id).RemoveAll()) {
        ...
        //        _logger.LogError(ex.Message);
        //        ModelState.AddModelError(string.Empty, Strings.ERROR_SERVER);
        //        strRenderView = await _renderView.RenderAsync(ModalConfiguration.IndexPath, new GetAll<IndicesModel>(), new ViewDataDictionary(ViewData));
        //        return await Task.FromResult(new JsonResult(strRenderView) { StatusCode = StatusCodes.Status500InternalServerError });
        //    }
        //}
