...
namespace Genealogy.WinFormsApp.Forms {
	...
	public partial class FrmRecursos : Form {
		...
		private void LoadDetails() {
			if (CustomTable.TableData.Rows[0].Cells[0].Value == null) {
				GbxData.SetControlsToReadOnly(false);
			} else {
				...
				TxtName.Text = CustomTable.GetSelectedCellValue(MappingsDB.Columna_Nombre)?.ToString();
				TxtDescription.Text = CustomTable.GetSelectedCellValue(MappingsDB.Columna_Descripcion)?.ToString();
				TxtTown.Text = CustomTable.GetSelectedCellValue(MappingsDB.Columna_Pueblo)?.ToString();
				TxtProvince.Text = CustomTable.GetSelectedCellValue(MappingsDB.Columna_Provincia)?.ToString();
