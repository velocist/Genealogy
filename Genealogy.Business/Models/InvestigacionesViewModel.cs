using Genealogy.Business.Models.App;
...
namespace Genealogy.Business.Models {
	...
	/// </summary>
	/// <seealso cref="Business.BaseModel&lt;InvestigacionesViewModel, AppEntitiesContext&gt;" />
	public class InvestigacionesViewModel : BaseModel<InvestigacionesViewModel, AppEntitiesContext> {
		...
		public InvestigacionesViewModel() : base() {
			//Logger = LogService.LogServiceContainer.GetLog<InvestigacionesViewModel>();
		}
		...
		public InvestigacionesViewModel Get() {
			try {
				Logger.LogDebug("Get<{TEntity},{TContext}>", typeof(InvestigacionesViewModel).Name, typeof(AppEntitiesContext).Name);
				var obj = UnitOfWork.GetRepository<Investigacion>().GetByID(InvestigacionModel.Id);
				if (obj != null)
		...
		public IEnumerable<InvestigacionesViewModel> List() {
			try {
				Logger.LogDebug("List<{TEntity},{TContext}>", typeof(InvestigacionesViewModel).Name, typeof(AppEntitiesContext).Name);
		...
		public bool Save() {
			try {
				Logger.LogDebug("Save<{TEntity},{TContext}>", typeof(InvestigacionesViewModel).Name, typeof(AppEntitiesContext).Name);
				var objetoDB = JsonAppHelper<Investigacion>.GetEntityFromObject(this);
				UnitOfWork.BeginTransaction();
				_ = UnitOfWork.GetRepository<Investigacion>().Insert(objetoDB);
		...
		public bool Update() {
			try {
				Logger.LogDebug("Update<{TEntity},{TContext}>", typeof(InvestigacionesViewModel).Name, typeof(AppEntitiesContext).Name);
				var objetoDB = JsonAppHelper<Investigacion>.GetEntityFromObject(this);
				UnitOfWork.BeginTransaction();
				_ = UnitOfWork.GetRepository<Investigacion>().Insert(objetoDB);
				UnitOfWork.CommitTransaction();
		...
		public bool Delete() {
			try {
				Logger.LogDebug("Delete<{TEntity},{TContext}>", typeof(InvestigacionesViewModel).Name, typeof(AppEntitiesContext).Name);
