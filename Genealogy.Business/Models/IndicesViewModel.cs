namespace Genealogy.Business.Models {
	...
	public class IndicesViewModel : BaseModel<IndicesViewModel, AppEntitiesContext> {
		...
		public IndicesViewModel() : base() {
			Logger = LogServiceContainer.GetLog<IndicesViewModel>();
		}
		...
		public IndicesViewModel Get() {
			try {
				Logger.LogDebug("Get<{TEntity},{TContext}>", typeof(IndicesViewModel).Name, typeof(AppEntitiesContext).Name);
		...
		public IEnumerable<IndicesViewModel> List() {
			try {
				Logger.LogDebug("List<{TEntity},{TContext}>", typeof(IndicesViewModel).Name, typeof(AppEntitiesContext).Name);
		...
		public bool Save() {
			try {
				Logger.LogDebug("Save<{TEntity},{TContext}>", typeof(IndicesViewModel).Name, typeof(AppEntitiesContext).Name);
		...
		public bool Update() {
			try {
				Logger.LogDebug("Update<{TEntity},{TContext}>", typeof(IndicesViewModel).Name, typeof(AppEntitiesContext).Name);
				var objetoDB = JsonAppHelper<Indices>.GetEntityFromObject(this);
				UnitOfWork.BeginTransaction();
				_ = UnitOfWork.GetRepository<Indices>().Insert(objetoDB);
				UnitOfWork.CommitTransaction();
		...
		public bool Delete() {
			try {
				Logger.LogDebug("Delete<{TEntity},{TContext}>", typeof(IndicesViewModel).Name, typeof(AppEntitiesContext).Name);
